...
OptixSceneCommitResult OptixScene::commit()
{
    OptixSceneCommitResult res;
...
OptixSceneCommitResult OptixScene::buildGAS()
{
    OptixSceneCommitResult res;
...
OptixSceneCommitResult OptixScene::buildIAS()
{
    OptixSceneCommitResult res;
    ...
    res.depth_changed = (depth_ != m_depth);
    res.sbt_size_changed = (m_required_sbt_entries != required_sbt_entries);
...
void OptixScene::addEventReceiver(OptixSceneEventReceiverPtr rec)
{
    OptixScenePtr scene_old = rec->m_scene.lock();
    OptixScenePtr scene_new = this_shared<OptixScene>();

    if(scene_new != scene_old)
    {
        if(scene_old)
        {
            // remove from old scene
            scene_old->removeEventReceiver(rec);
        }

        rec->m_scene = scene_new;
    } else {
        // already added? must be in m_event_rec
    }

    m_event_rec.insert(rec);
}

void OptixScene::removeEventReceiver(OptixSceneEventReceiverPtr rec)
{
    m_event_rec.erase(rec);
}
...
void OptixScene::notifyEventReceivers(const OptixSceneCommitResult& info)
{
    ...
    if(info.depth_changed)
    {
        ...
        {
            OptixSceneEventReceiverPtr rec = rec_it->lock();
            if(rec)
            {
                rec->onDepthChanged();
                ++rec_it;
    ...
    for(auto rec_it = m_event_rec.begin(); rec_it != m_event_rec.end();)
    {
        ...
        if(rec)
        {
            rec->onSBTUpdated(info.sbt_size_changed);
            ++rec_it;
        } else {
            m_event_rec.erase(rec_it);
        }
