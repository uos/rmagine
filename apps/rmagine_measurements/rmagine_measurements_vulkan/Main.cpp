...
int main(int argc, char** argv)
{
    ...
    //measure different num of sensors
    {
        ...
        for(size_t i = 0; i <= num_tbms; i++)
        {
            ...
            if(i == 0)
                tbm_size = 1;
            Memory<Transform, RAM> tbm_ram(tbm_size);
    ...
    //measure different num of faces
    {
        ...
        for(size_t i = 0; i <= num_maps; i++)
        {
            ...
            double elapsed_avg = measure(tbm_ram, tbm, res, map, sim_gpu_sphere);
            std::stringstream point;
            point << "(" << static_cast<double>(map->scene()->geometries().begin()->second->this_shared<VulkanInst>()->scene()->geometries().begin()->second->this_shared<VulkanMesh>()->faces.size())/1000000.0 << ", " << elapsed_avg << ")";
            results.push_back(point.str());
            std::cout << "\n" << std::endl;
        }
        for(size_t i = 0; i < results.size(); i++)
